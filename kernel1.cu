                  ...
                  int32_t *output, int32_t width, int32_t height)
{
    ...
    // kernel 1
    kernel1<<<num_blocks, num_threads>>>(filter, dimension, input, output, width, height);
    ...
    // normalize 1
    normalize1<<<num_blocks, num_threads>>>(output, width, height, global_min, global_max);
    cudaFree(global_min);
    cudaFree(global_max);
}
        ...
        const int32_t *input, int32_t *output, int32_t width, int32_t height)
{
    ...
    if (location < width * height && idx < width * height){
        output[location] = apply2d_gpu(filter, dimension, input, width, height, row, column);
    }
}
        ...
        int32_t *smallest, int32_t *biggest)
{
    ...
    if (location < width * height){
        if (smallest[0] != biggest[0]) {
            image[location] = ((image[location] - smallest[0]) * 255) / (biggest[0] - smallest[0]);
        }
    }
}
